...
class VMwareProvider(Provider):
    ...
    def start_emulator(self, path_to_vm: str, headless: bool, os_type: str):
        ...
        os.environ["PATH"] += r";D:\work_app\vmware\vmrun.exe"
        os.environ["PATH"] += r";D:\work_app\vmware"
        while True:
            try:
                ...
                logger.error(f"Error executing command:{e.output.decode('gbk', errors='ignore').strip()}")
    ...
    def get_ip_address(self, path_to_vm: str) -> str:
        ...
        while True:
            try:
                ...
                # )
                # output = VMwareProvider._execute_command(
                #     ["vmrun"] + get_vmrun_type(return_list=True) + ["runProgramInGuest", path_to_vm, "-activeWindow", "/bin/bash", "-c", "hostname -I"],
                #     return_output=True
                # )
                # print(output)
                # ubuntu
                output = '192.168.50.122'
                # windows
                # output = '192.168.127.128'
