...
## [Ubuntu](https://huggingface.co/datasets/Chenhangcui/ubuntu_RAGE)
...
### Set up the RAGE server service in VM
...
3. Copy the `RAGE_server.service` to the systemd configuration directory at `/etc/systemd/system/`:
    ```shell
    sudo cp RAGE_server.service /etc/systemd/system/
    ```
   ...
   Enable the service to start on boot:
    ```shell
    sudo systemctl enable RAGE_server.service
    ```

   Start the service:
    ```shell
    sudo systemctl start RAGE_server.service
    ```

   Verify the service is running correctly:
    ```shell
    sudo systemctl status RAGE_server.service
    ```
   ...
   If you need to make adjustments to the service configuration, you can edit the `/etc/systemd/system/RAGE_server.service` file:
    ```shell
    sudo nano /etc/systemd/system/RAGE_server.service
    ```
...
```bash
1. Download the extension from: https://github.com/xlang-ai/RAGE/blob/04a9df627c7033fab991806200877a655e895bfd/vscodeEvalExtension/eval-0.0.1.vsix
2. Open VS Code
...
The required screen resolution for the virtual machine is 1920x1080 in RAGE and we did make some hardcode related to this resolution in our configuration file in some examples, but only a few. 
...
## [Windows](https://huggingface.co/datasets/Chenhangcui/windows_RAGE)
Coming soon...

## [MacOS](https://huggingface.co/datasets/Chenhangcui/macos_RAGE)
Coming soon...
