...
class PromptAgent:
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        if self.observation_type in ["screenshot", "screenshot_a11y_tree"]:
            ...
            linearized_accessibility_tree = linearized_accessibility_tree.replace("\u200b", "")
    ...
    def call_llm(self, payload):
        if self.model.startswith("o3") and "max_tokens" in payload:
            payload["max_completion_tokens"] = payload.pop("max_tokens")
            payload.pop("top_p")
        if self.model.startswith("gpt") or self.model.startswith("o3"):
            headers = {
            ...
            if response.status_code == 200:
                ...
                return ""
        elif self.model in ["qwen-max",'qwen-turbo', 'qwen-plus',"gemini-1.5-pro", "gemini-pro", "gemini-pro-vision", "gemini-2.0-flash"] or self.model.startswith("claude") or self.model.startswith("deepseek"):
            # print('##############################')
...
class PromptAgentwoaction:
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        if self.observation_type in ["screenshot", "screenshot_a11y_tree"]:
            ...
            # Add som to the screenshot
            masks, drew_nodes, tagged_screenshot, linearized_accessibility_tree = tag_screenshot(obs["screenshot"],
                                                                                                 obs[
                                                                                                     "accessibility_tree"],
                                                                                                 self.platform)
            base64_image = encode_image(tagged_screenshot)
